...
4) **Vendor moshi code (no GitHub dependency)**
...
5) **Define update strategy for vendored moshi**
...
7) **Observability and runtime options**
...
9) **Environment requirements (Aspire-aligned)**
...
## Open Questions
- Should the frontend remain inside FastAPI (single service) or be split into a static frontend + API service?
- Should the frontend get moshi endpoint info via a small config API or via static config (e.g., env-injected JSON)?
- What is the preferred local-dev HTTPS/WSS cert approach (self-signed acceptance vs trusted dev certs)?
- How strict do we want the vendoring process to be (manual copy vs scripted update)?
